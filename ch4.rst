...
``:initial-element`` 參數是選擇性的。如果提供了這個參數，整個陣列會用指定的值作初始化。嘗試從一個未初始化的陣列內取出元素的行為，其後果未定義 (undefined)。
...
第二個例子我們要求在第四個與第六個字元間，第一個 ``a`` 所出現的位置。 ``:start`` 關鍵字參數是第一個被考慮的元素位置，預設是序列的第一個元素。 ``:end`` 關鍵字參數，如果有給的話，是第一個不被考慮的元素位置。
