...
作為一個序列操作的例子，這小節演示了如何寫一個程式來解析日期。我們將編寫一個程式，可以接受一個像是 "16 Aug 1980" 的字串，然後回傳一個表示日、月、年的整數列表。
...
函數 ``constituent`` 被定義成用來作為 ``tokens`` 的參數。

在 Common Lisp 裡， *圖形字元* 是我們可見的字元，加上空白字元。所以如果我們用 ``constituent`` 作為測試函數時，
