...
由於 ``cond`` 子句的 ``t`` 条件永远成立，通常我们把它放在最後，作为预设的条件式。如果没有子句符合时，则 ``cond`` 返回 ``nil`` ，但利用 ``nil`` 返回值是一种很差的风格 (这种问题可能发生的例子，请看 292 页)。译注: **Appendix A, unexpected nil** 小节。
...
预设子句的键值可以是 ``t`` 或 ``otherwise`` 。如果没有子句符合时，或是子句只包含键值时，
...
当迭代结束时，初始列表内的第三个元素 (译注: ``done`` ) ，会被求值并作为 ``dolist`` 的返回值。预设是 ``nil`` 。
...
使用 ``dolist`` ，初始列表内的第三个元素是选择性的，预设是 ``nil`` 。注意它可以参照到迭代里的变量。
