...
那麽这个字符串会变成函数的文档字符串（documentation string）。要取得函数的文档字符串，可以通过调用 ``documentation`` 来取得：
...
如果我们在函数的形参列表里的最后一个变量前，插入 ``&rest`` 符号，那麽当这个函数被调用时，这个变量会被设成一个带有剩余参数的列表。现在我们可以明白 ``funcall`` 是如何根据 ``apply`` 写成的。它或许可以定义成：
