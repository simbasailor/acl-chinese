...
比值与复数概念上是两部分的结构。(译注：像 **Cons** 这样的两部分结构) 函数 ``numerator`` 与 ``denominator`` 返回比值或整数的分子与分母。（如果数字是整数，前者返回该数，而後者返回 ``1`` 。）函数 ``realpart`` 与 ``imgpart`` 返回任何数字的实数与虚数部分。（如果数字不是复数，前者返回该数字，後者返回 ``0`` 。）
...
最後 ``minroot`` 接受三个实数， ``a`` , ``b`` 与 ``c`` ，并返回满足等式 :math:`ax^2+bx+c=0` 的最小实数 ``x`` 。当 ``a`` 不为 :math:`0` 时，这个等式的根由下面这个熟悉的式子给出:
