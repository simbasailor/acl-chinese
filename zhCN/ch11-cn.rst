...
Common Lisp 对象系统，或称为 CLOS，是一组用来实作面向对象编程的操作集。由于它们有著同样的历史，通常将这些操作视为一个群组。 `λ <http://acl.readthedocs.org/en/latest/zhCN/notes-cn.html#notes-176>`_ 技术上来说，它们与其他部分的 Common Lisp 没什么大不同： ``defmethod`` 和 ``defun`` 一样，都是整合在语言中的一个部分。
...
方法可以像一般 Common Lisp 函数一样有复杂的参数列表，但所有组成通用函数方法的參數列表必须是一致的 (\ *congruent*\ )。参数的数量必须一致，同样数量的选择性参数（如果有的话），要嘛一起使用 ``&rest`` 或是 ``&key`` 参数，或者一起不要用。下面的参数列表对是全部一致的，
