...
我们通常在别的语言找不到的两个 Lisp 数据类型是 *符号* (symbol)与 *列表* (lists)， *符号* 是单词 (words)。无论你怎么输入，通常它们被转换成大写：
...
``setf`` 的第一个参数几乎可以是任何参照到特定位置的表达式。所有这样的操作符在附录 D 中被标注为 “可设置的” ("settable")。你可以给任何（偶数）数目的参数至 ``setf`` 。一个这样的表达式
