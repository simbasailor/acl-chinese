...
许多表达式由预设子句 (default clause)来处理，预设子句获得 ``car`` 所引用的函数，将 ``cdr`` 所有的参数求值，并返回将前者应用至後者的结果。 [1]_
...
图 10.2 的最後一个宏是 ``aif`` ，它在此作为一个故意变量捕捉的例子。它让我们可以使用变量 ``it`` 来引用到一个条件式里的测试参数所返回的值。也就是说，与其写成
