...
圖 12.8 展示瞭如何定義一個具有破壞性的 ``bst-insert`` (第 72 頁「\ **譯者注：**\ 第 4.7 節」)。相同的輸入參數，能夠得到相同返回值。唯一的區別是，它將修改作爲第二個參數輸入的 BST。 在第 2.12 節中說過，具有破壞性並不意味着一個函數呼叫具有副作用。的確如此，如果你想使用 ``bst-insert!`` 構造一個 BST，你必須像呼叫 ``bst-insert`` 那樣呼叫它：
