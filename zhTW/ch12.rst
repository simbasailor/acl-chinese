...
當你調用別人寫的函數的時候要加倍小心。除非你知道它內部的操作，否則，你傳入的參數時要考慮到以下的情況：

1.它對你傳入的參數可能會有破壞性的操作

2.你傳入的參數可能被保存起來，如果你調用了一個函數，然後又修改了之前作爲參數傳入該函數的物件，那麼你也就改變了函數已保存起來作爲它用的物件[1]。

在這兩種情況下，解決的方法是傳入一個拷貝。

在 Common Lisp 中，一個函數調用在遍歷列表結構 (比如， ``mapcar`` 或 ``remove-if`` 的參數)的過程中不允許修改被遍歷的結構。關於評估這樣的
...
Common Lisp 包含一些允許修改列表結構的函數。爲了提高效率，這些函數是具有破壞性的。雖然它們可以回收利用作爲參數傳給它們的 ``cons`` ，但並不是因爲想要它們的副作用而調用它們 (\ **譯者注：**\ 因爲這些函數的副作用並沒有任何保證，下面的例子將說明問題)。
...
這個函數調用 ``copy-list`` 時存在一個假設  —— ``chlidren`` 函數返回的是一個已經保存在某個地方的列表，而不是構建了一個新的列表。
...
圖 12.8 展示瞭如何定義一個具有破壞性的 ``bst-insert`` (第 72 頁「\ **譯者注：**\ 第 4.7 節」)。相同的輸入參數，能夠得到相同返回值。唯一的區別是，它將修改作爲第二個參數輸入的 BST。 在第 2.12 節中說過，具有破壞性並不意味着一個函數調用具有副作用。的確如此，如果你想使用 ``bst-insert!`` 構造一個 BST，你必須像調用 ``bst-insert`` 那樣調用它：
...
與 ``bst-remove`` (第 74 頁「\ **譯者注：**\ 第 4.7 節」) 對應，圖 12.9 展示了一個破壞性版本的 ``bst-delete`` 。同 ``delete`` 一樣，我們調用它並不是因爲它的副作用。你應該像調用 ``bst-remove`` 那樣調用 ``bst-delete`` ：
...
普通的 Lisp 列表是單向鏈表，這意味着其指標指向一個方向：我們可以獲取下一個元素，但不能獲取前一個。在\ `雙向鏈表 <http://zh.wikipedia.org/wiki/%E5%8F%8C%E5%90%91%E9%93%BE%E8%A1%A8>`_\ 中，指標指向兩個方向，我們獲取前一個元素和下一個元素都很容易。這一節將介紹如何創建和操作雙向鏈表。
...
如果 Lisp 試着打印我們剛剛構造的結構，將會顯示 (a a a a a …… —— 無限個 ``a``)。但如果設置全局變數 ``*print-circle*`` 爲 ``t`` 的話，Lisp 就會採用一種方式打印出一個能代表環形結構的物件：
...
有幾個其它方法來實現 ``arith-op``，使其不返回被引用列表的部分。一般地，我們可以通過將其中的所有引用( ``quote`` ) 替換成 ``list`` 來確保安全，這使得它每次被調用都將返回一個新的列表：
