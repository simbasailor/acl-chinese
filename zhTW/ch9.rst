...
最後 ``minroot`` 接受三個實數， ``a`` , ``b`` 與 ``c`` ，並返回滿足等式 :math:`ax^2+bx+c=0` 的最小實數 ``x`` 。當 ``a`` 不爲 :math:`0` 時，這個等式的根由下面這個熟悉的式子給出:
...
``surface`` 結構用來表示模擬世界中的物體。更精確的說，它會被 ``included`` 至定義具體型態物體的結構裡，像是球體 (spheres)。 ``surface`` 結構本身只包含一個欄位: 一個 ``color`` 範圍從 0 (黑色) 至 1 (白色)。
