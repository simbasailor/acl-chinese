...
在給定的實現裡，用十六個全局常數標明瞭每個格式的限制。它們的名字是這種形式: ``m-s-f`` ，其中 ``m`` 是 ``most`` 或 ``least`` ， ``s`` 是 ``positive`` 或 ``negative`` ，而 ``f`` 是四種浮點數之一。 `λ <http://acl.readthedocs.org/en/latest/zhTW/notes.html#notes-150>`_
