...
處理數字是 Common Lisp 的強項之一。Common Lisp 有著豐富的數值型態，而 Common Lisp 操作數字的特性與其他語言比起來更受人喜愛。
...
函數 ``random`` 接受一個整數或浮點數。這樣形式的表達式 ``(random n)`` ，會返回一個大於或小於等於 ``n`` 的數字，並有著與 ``n`` 相同的型態。
...
(譯註：PGM 可移植灰度圖格式，更多資訊參見 `wiki <http://en.wikipedia.org/wiki/Portable_graymap>`_ )
